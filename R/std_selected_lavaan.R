...
gen_std_i <- function(fit,
    ...
    out <- function(par) {
        ...
        # TODO:
        # - Factor out the following section

        # Regression Path
        # if (op == "~") {
        #     if (length(prod_names) > 0) {
        #         # Involve Product Terms
        #         if (lhs %in% prod_names) {
        #             # x-term?
        #             if (prods[[lhs]]$x %in% to_standardize_i) {
        #                 std_by <- c(std_by, prods[[lhs]]$x)
        #                 std_out_i$est <- std_out_i$est / fit_sd_all[prods[[lhs]]$x]
        #               }
        #             # w-term?
        #             if (prods[[lhs]]$w %in% to_standardize_i) {
        #                 std_by <- c(std_by, prods[[lhs]]$w)
        #                 std_out_i$est <- std_out_i$est / fit_sd_all[prods[[lhs]]$w]
        #               }
        #           } else {
        #             # Product term not in LHS
        #             if (lhs %in% to_standardize_i) {
        #                 std_by <- c(std_by, lhs)
        #                 std_out_i$est <- std_out_i$est / fit_sd_all[lhs]
        #               }
        #           }
        #         if (rhs %in% prod_names) {
        #             # x-term?
        #             if (prods[[rhs]]$x %in% to_standardize_i) {
        #                 std_by <- c(std_by, prods[[rhs]]$x)
        #                 std_out_i$est <- std_out_i$est * fit_sd_all[prods[[rhs]]$x]
        #               }
        #             # w-term?
        #             if (prods[[rhs]]$w %in% to_standardize_i) {
        #                 std_by <- c(std_by, prods[[rhs]]$w)
        #                 std_out_i$est <- std_out_i$est * fit_sd_all[prods[[rhs]]$w]
        #               }
        #           } else {
        #             # Product term not in RHS
        #             if (rhs %in% to_standardize_i) {
        #                 std_by <- c(std_by, rhs)
        #                 std_out_i$est <- std_out_i$est * fit_sd_all[rhs]
        #               }
        #           }
        #       } else {
        #         # No Product Term
        ...
        # if (op == "~~") {
        #     # TODO
        #     # - Decide what to do with "~~" involving product terms
