...
To standardize a variablex we first mean center it and then scale it by its
...
Note that we can also mean center or standardize the dependent variable. We
...
that `std_selected()` will skip categorical variables (i.e., factors or string
...
Currently, only the 95% confidence intervals will be reported.
...
Note: As a side product, the nonparametric Bootstrapping confidence of the
