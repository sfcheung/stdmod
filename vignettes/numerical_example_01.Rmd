...
This data set has 500 cases of data. The variables are sleep duration, age, gender,  and the scores from two personality scales, emotional stability and conscientioisnes of the IPIP Big Five markers. Please refer to xxx for the detail of the data set.
...
```{r mod_reg}
lm_raw <- lm(sleep_duration ~ age + gender + emotionality_stability*conscientiousness, sleep_emo_con)
summary(lm_raw)
visreg(lm_raw, "emotionality_stability", "conscientiousness",
...
visreg(lm_w_centered, "emotionality_stability", "conscientiousness",
            breaks = 2, overlay = TRUE)
```
...
```{r}
lm_xw_centered <- std_selected(lm_raw, to_center = ~ emotionality_stability + conscientiousness)
summary(lm_xw_centered)
visreg(lm_xw_centered, "emotionality_stability", "conscientiousness",
            breaks = 2, overlay = TRUE)
```
...
```{r}
lm_xw_std <- std_selected(lm_raw, to_center = ~ emotionality_stability + conscientiousness,
                                  to_scale  = ~ emotionality_stability + conscientiousness)
summary(lm_xw_std)
visreg(lm_xw_std, "emotionality_stability", "conscientiousness",
            breaks = 2, overlay = TRUE)
```
...
```{r}
lm_xwy_std <- std_selected(lm_raw, to_center = ~ emotionality_stability + conscientiousness + sleep_duration,
                                  to_scale  = ~ emotionality_stability + conscientiousness + sleep_duration)
summary(lm_xwy_std)
visreg(lm_xwy_std, "emotionality_stability", "conscientiousness",
            breaks = 2, overlay = TRUE)
```
...
lm_all_std <- std_selected(lm_raw, to_center = ~ .,
                                   to_scale  = ~ .)
summary(lm_all_std)
visreg(lm_all_std, "emotionality_stability", "conscientiousness",
            breaks = 2, overlay = TRUE)
```
...
```{r}
lm_xwy_std_ci <- std_selected_boot(lm_raw, to_center = ~ emotionality_stability + conscientiousness + sleep_duration,
                                           to_scale  = ~ emotionality_stability + conscientiousness + sleep_duration,
                                           nboot = 2000)
