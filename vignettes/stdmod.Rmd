...
The unstandardized moderation effect is significant, B = `r round(coef(lm_out)["emot:cons"], 4)`.
For each one unit increase of conscientiousness score, the effect of emotional
stability decreases by `r round(-1 * coef(lm_out)["emot:cons"], 4)`.
...
Standardization is equivalent to centering by mean and then scaling by (dividing
by) standard deviation. `to_center` specifies the variables to be centered
by means, and `to_scale` specifies the variables to be scaled by standard
deviation. The formula interface of `lm()` is used in these two arguments,
...
The **standardized moderation effect** is significant, B = `r round(coef(lm_stdall)["emot:cons"], 4)`.
For each one *standard deviation* increase of conscientiousness score, the
**standardized effect** of emotional stability decreases by `r round(-1 * coef(lm_stdall)["emot:cons"], 4)`.

## Nonparametric Bootstrapping Confidence Intervals

The confidence intervals based on OLS do not take into account the sampling variation of
the sample means and standard deviations used in standardization. Cheung, Cheung, Lau, Hui,
and Vong (2022) suggest using nonparametric bootstrapping, with standardization conducted
in each bootstrap sample.

This can be done by `std_selected_boot()` instead of `std_selected()` and add
...
emotional stability and conscientiousness. We list them on `to_center` and
`to_scale`:
...
(`std_selected()` and `std_selected_boot()`)).
...
Advance online publication. *Health Psychology*. [DOI to be included].